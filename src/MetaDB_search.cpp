...
void printhelp(){
     ...
     cout << "Usage : MetaDB-search [Options] Value" << endl;
...
int main(int argc, char * argv[]){
    ...
    _MetaDB_search metadb_searcher;
    ...
    if (Mem_mode == 1){
		...
		metadb_searcher.Set_Sample_Hdd_Prefix(DB_prefix);
   	    if (!Check_Path(DB_prefix.c_str())){
   	    	...
   	    	cerr << "Please make the hdd data by MetaDB-make, or disable the HDD mode (-H F)" << endl;
   	    	return 0;
		   }
		}
    ...
    case 0: //Single
            {
                ...
                if (Is_index)
                    res.push_back(metadb_searcher.Index_Search(Singlefilename.c_str(), Query_N, Query_f, Index_t, Sim_t, Is_weight, Coren));
                else
                    res.push_back(metadb_searcher.Search(Singlefilename.c_str(), Query_N, Sim_t, Is_weight, Coren));
            }
            break;
    ...
    case 1: //List
            {    
                ...
                if (Is_index)
                    res = metadb_searcher.Index_Search(queryfiles, Query_N, Query_f, Index_t, Sim_t, Is_weight, Coren);
                else
                    res = metadb_searcher.Search(queryfiles, Query_N, Sim_t, Is_weight, Coren);
            }
            break;
    ...
    case 2: //Tableformat
            {
                ...
                if (Is_index)
                    res = metadb_searcher.Index_Search(&table, Query_N, Query_f, Index_t, Sim_t, Is_weight, Coren);
                else
                    res = metadb_searcher.Search(&table, Query_N, Sim_t, Is_weight, Coren);
            }
            break;
