...
void printhelp(){
    ...
    cout << "Usage : MetaDB-make-sp [Options] Value" << endl;
    
    cout << "\t[Input options]" << endl;
    cout << "\t  -i or -l Input filename list" << endl;
    cout << "\t  -p List file path prefix for '-i' or '-l' [Optional for -i and -l]" << endl;
    cout << "\tor" << endl;
    cout << "\t  -T (upper) Input OTU table (*.OTU.Count)" << endl;
    cout << "\tor" << endl;
    cout << "\t  -d (*.mdbs) Make the HDD mode data files for a database" << endl;
    
    cout << endl <<  "\t[Output options]" << endl;
    cout << "\t  -o Output database name, default is \"database.mdbs\"" << endl;
    ...
    int i = 1;
      ...
      while(i<argc){
         ...
         switch(argv[i][1]){
                            ...
                            case 'o': Outfilename = argv[i+1]; Outfilename += ".mdbs"; break;
...
int main(int argc, char * argv[]){
    ...
    switch (Mode){
           case 0:  //List
                    count = metadb_maker.Make_Database(Outfilename.c_str(), DB_Listfilename, Listprefix, Is_cp_correct); 
                    break;
           case 1: //Table
                   {
                   _Table_Format table(DB_Tablefilename.c_str());
                   count = metadb_maker.Make_Database(Outfilename.c_str(), &table, Is_cp_correct);
                   }
                   break;
           case 3: //Hdd only
