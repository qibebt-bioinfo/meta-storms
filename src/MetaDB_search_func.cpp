...
int Mode = 0; //0:singlefile; 1: listfile; 2: tablefile
...
void printhelp(){
     ...
     cout << "Usage : MetaDB-search-func [Options] Value" << endl;
          
     cout << "\t[Database options]" << endl;
     cout << "\t  -d Database file (*.mdbf) [Required]" << endl;
     ...
     cout << "\t  -o Output file, default is \"query.func.out\"" << endl;
     ...
     cout << "\t  -e If enable the exhaustive search (high accuracy, low speed), T(rue) or F(alse), default is F" << endl;
     cout << "\t  -M (upper) Distance Metric, 0: Meta-Storms-Func; 1: Cosine; 2: Euclidean; 3: Jensen-Shannon, 4: Bray-Curtis; default is 0" << endl;
...
int Parse_Para(int argc, char * argv[]){
    ...
    int i = 1;
      ...
      while(i<argc){
         ...
         switch(argv[i][1]){
                            ...
                            //case 'M': Out_format = atoi(argv[i+1]); break;
    ...
    if (DB_filename.size() < 0){
                           cerr << "Error: Please assign the database (.mdbf) by -d" << endl;
                           exit(0);
                           }
...
int main(int argc, char * argv[]){
    ...
    _MetaDB_search metadb_searcher('F');
    ...
    if (Mem_mode == 1){
		...
		metadb_searcher.Set_Sample_Hdd_Prefix(DB_prefix);
   	    if (!Check_Path(DB_prefix.c_str())){
   	    	...
   	    	cerr << "Please make the hdd data by MetaDB-make-func, or disable the HDD mode (-H F)" << endl;
   	    	return 0;
		   }
		}
