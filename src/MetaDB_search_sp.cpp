...
void printhelp(){
     ...
     cout << "Usage : MetaDB-search-sp [Options] Value" << endl;
     cout << "\t[Database options]" << endl;
     cout << "\t  -d Database file (*.mdbs) [Required]" << endl;
...
int main(int argc, char * argv[]){
    ...
    _MetaDB_search metadb_searcher('M');
    ...
    if (Mem_mode == 1){
		...
		metadb_searcher.Set_Sample_Hdd_Prefix(DB_prefix);
   	    if (!Check_Path(DB_prefix.c_str())){
   	    	...
   	    	cerr << "Please make the hdd data by MetaDB-make-sp, or disable the HDD mode (-H F)" << endl;
   	    	return 0;
		   }
		}
